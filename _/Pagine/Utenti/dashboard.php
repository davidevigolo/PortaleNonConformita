...
<body>
    <?php 
        ...
        $q = "SELECT S.ID as COD,S.STATO,S.DATACREAZIONE,S.DATACHIUSURA,A.USERNAME AS AUTORE 
                FROM SEGNALAZIONE S JOIN NONCONFORMITA N ON S.TIPO=N.ID JOIN ACCOUNT AS A ON S.AUTORE=A.IDSEGNALANTE 
                WHERE S.AUTORE={$_SESSION['idsegn']} AND A.USERNAME='{$_SESSION['username']}'
                ORDER BY S.ID";
        ...
        echo '<div class="nc"><table class="actions">
            ...
            while($row = mysqli_fetch_assoc($risultato)){
                ...
                echo "<tr><td>$row[COD]</td><td>$row[STATO]</td><td>$row[DATACREAZIONE]</td><td>$row[DATACHIUSURA]</td><td>$row[AUTORE]</td><td><form method=\"GET\" action=\"dettagliNC.php\"><input type=submit value=\"Dettagli\"><input type=\"hidden\" name=\"COD\" value=$row[COD]></td></form></tr>";
            }
            //questa parte servirà a mostrare le N.C. in cui si è coinvolti ma che non si sono necessariamente create
            $q = "SELECT G.IDSEGNALAZIONE,S1.DATACREAZIONE,S1.DATACHIUSURA,A.USERNAME,S1.ID FROM GESTIONENC G JOIN SEGNALAZIONE S1 ON G.IDSEGNALAZIONE=S1.ID JOIN ACCOUNT A ON S1.AUTORE=A.IDSEGNALANTE WHERE G.IDSEGNALANTE=$_SESSION[idsegn] AND S1.AUTORE <> $_SESSION[idsegn]";
            $risultato = mysqli_query($connessione, $q);
