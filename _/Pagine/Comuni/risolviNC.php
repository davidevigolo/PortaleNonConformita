        ...
        <form method="POST" action="risolviNC.php" style="flex:100%;">
            <select name="idnc" class="selector" style="width:100%">
                ...
                if($_SESSION['role']=='Dirigente'||$_SESSION['role']=='Caporeparto'||$_SESSION['role']=='Admin'){
                    $nonconformitaq = "SELECT S.ID,NC.NOME,S.NCREPARTO,S.NCFORNITORE,S.AUTORE FROM SEGNALAZIONE S JOIN NONCONFORMITA NC ON S.TIPO=NC.ID WHERE NC.GRADOMINIMO <= $_SESSION[gradominimo] ORDER BY S.ID ASC";
                }else{
                    $nonconformitaq = "SELECT S.ID,NC.NOME,S.NCREPARTO,S.NCFORNITORE,S.AUTORE FROM SEGNALAZIONE S JOIN NONCONFORMITA NC ON S.TIPO=NC.ID WHERE NC.GRADOMINIMO <= $_SESSION[gradominimo] AND STATO='APERTA' ORDER BY S.ID ASC";
                }
